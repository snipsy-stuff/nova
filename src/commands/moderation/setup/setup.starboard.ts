    ...
    options: [
        ...
        new AutoComplete()
            ...
            .addAutoComplete(async (ctx) => {
                ...
                const filterd = ctx.guild.emojis
                    .filter((em) =>
                        em.name
                            .toLowerCase()
                    ...
                    .map((em) => ({
                        name: em.animated ? `a_${em.name}` : em.name,
                        value: em.id || em.name,
                    }))
                    .slice(0, 15);
                return ctx.respond({
        ...
        new NumberOption()
            .setName('count')
            .setDescription(
                'how many times this emoji must be reacted to a message.',
            ),
        new AutoComplete()
            .setName('react_emoji')
            .setDescription('what emoji you want to use')
        ...
        if (!existing || !existing.star_board) {
            ...
            existing = {
                ...
                star_board: {
                    ...
                    channel:
                        ctx.args.channel?.id ??
                        defaultSettings.star_board.channel,
                    ...
                    enabled:
                        ctx.args.enabled ||
                        defaultSettings.star_board.enabled,
                    messages: [],
            ...
            const emojis =
                existing.star_board.emojis.length && ctx.args.emoji
                    ? [
            ...
            existing.star_board = {
                ...
                channel:
                    ctx.args.channel?.id ??
                    existing.star_board.channel,
                emojis: emojis,
                messages: [...existing.star_board.messages],
                react_emoji:
                    ctx.args.react_emoji ||
                    existing.star_board.react_emoji,
            };
        }
